            ...
            <div class="row g-3 align-items-center">
                ...
                <div class="col-md-4">
                    <select name="jenjang_id" class="form-select rounded-4 py-2">
                        ...
                        @foreach($jenjangs as $jenjang)
                            <option value="{{ $jenjang->id }}" {{ request('jenjang_id') == $jenjang->id ? 'selected' : '' }}>
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    @if(request()->filled('search') || request()->filled('jenjang_id'))
                        <a href="{{ route('pengajuan.index') }}" class="btn btn-light rounded-pill px-3 text-secondary" title="Reset Filter">
                    ...
                    <tbody>
                        @forelse ($pengajuans as $item)
                            <tr>
                                ...
                                <td><span class="text-primary fw-bold">Rp {{ number_format($item->total_biaya, 0, ',', '.') }}</span></td>
                                ...
                                <td class="text-center pe-4">
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('pengajuan.show', $item->id) }}" class="btn-action btn btn-info text-white" title="Lihat"><i class="fa-solid fa-eye"></i></a>
                                        @if ($item->status == 'Pending')
                                            <a href="{{ route('pengajuan.edit', $item->id) }}" class="btn-action btn btn-warning text-white" title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>
                                        @endif
                                        <a href="{{ route('pengajuan.cetak', $item->id) }}" class="btn-action btn btn-danger" target="_blank" title="PDF"><i class="fa-solid fa-file-pdf"></i></a>
                                        @if ($item->status == 'Pending' || auth()->user()->role->slug == 'admin')
                                            <form action="{{ route('pengajuan.destroy', $item->id) }}" method="POST" class="d-inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="btn-action btn btn-outline-danger" onclick="return confirm('Yakin hapus?')" title="Hapus"><i class="fa-solid fa-trash"></i></button>
                                            </form>
                                        @endif
                                    </div>
                                </td>
